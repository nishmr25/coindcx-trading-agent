      ...
      <div style={{ padding: "30px 25px" }}>
        {activeTab === "wallet" && (         
          <div style={{ background: C.panel, padding: 25, borderRadius: 10, border: `1px solid ${C.border}` }}>
            <h3 style={{ marginBottom: 20, color: C.accent }}>Wallet Information</h3>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "15px 0", borderBottom: `1px solid ${C.border}44` }}>
              <span style={{ color: C.textDim }}>Total Balance</span>
              <span>{inr(balanceData.balance)}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "15px 0", borderBottom: `1px solid ${C.border}44` }}>
              <span style={{ color: C.textDim }}>Total Profit</span>
              <span style={{ color: C.bull }}>{inr(balanceData.totalProfit)}</span>
            </div>
            <div style={{ marginTop: 20 }}>
              <h4 style={{ color: C.accent, marginBottom: 10 }}>Recent Transactions</h4>
              {balanceData.transactions.length === 0 ? (
                <div style={{ color: C.textDim }}>No transactions yet.</div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {balanceData.transactions.map((tx, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                      <span style={{ color: tx.type === 'DEPOSIT' ? C.bull : C.bear }}>{tx.type}</span>
        ...
        {activeTab === "settings" && (         
          <div style={{ background: C.panel, padding: 30, borderRadius: 10, border: `1px solid ${C.border}`, maxWidth: 500, margin: "0 auto" }}>
            <h3 style={{ marginBottom: 20, color: C.accent }}>Settings</h3>
            <div style={{ color: C.textDim, fontSize: 14 }}>
              <p>This is a trading agent only version. Login, deposit, withdrawal, and 2FA features have been removed.</p>
              <p>The platform runs on a pooled fund managed by the automated trading engine.</p>
            </div>
          </div>
        )}
